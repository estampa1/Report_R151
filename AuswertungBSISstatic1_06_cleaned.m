...
% === Dateipfad ===
%filePath = "C:\Users\G00550\Desktop\Elias\Auswertung R151 Sprinter\schmaler korrigiert\Statisch 1\V4_T29_R2.txt";
%filePath = "C:\Users\G00550\Desktop\Elias\Auswertung R151 Sprinter\schmaler korrigiert\Statisch 1\R1\V4_T29_R1.txt";

% 2907
%filePath = "C:\Users\G00550\Desktop\Exporte\Statisch1\V9_T55_R3.txt";
filePath = "C:\Users\G00550\Desktop\Exporte\Statisch1\V9_T55_R2.txt";
%filePath = "C:\Users\G00550\Desktop\Exporte\Statisch1\V9_T55_R1.txt";
...
% bsFront = 1.9;              % m
% bsHeck = -3;                % m
targetStartDistance = -11.11;  % m
%xTargetDes = 0;
dc = -2;

scenarioString = string(sprintf("Static Test Case 1 (Crossing) - Target: %.1f km/h - lat. Distance: 1.15 m", speedTarget));
...
%% === Berechnung notwendiger Größen ===
    y_target = y_target_frontAxle + xRefTarget;
    if lateralExtension
        warning("Berechnungen berücksichtigen den seitlichen Ausleger!")
        x_target_cos = x_target_frontAxle - yRefTarget_Extension;
    else    
        x_target_cos = x_target_frontAxle - yRefTarget;
    end
    x_target = x_target_cos + 1.4;
    x_vut_cos = x_vut_frontAxle + xRefVUT;
    x_vut = x_vut_cos + 1.4;
    y_vut_cos = y_vut_frontAxle + yRefVUT;
    y_vut = y_vut_frontAxle;

%% === t₀-Ermittlung: Target hat Beschleunigungsweg 5,66 m zurückgelegt ===
y_target_start = y_target(1);
travelledDist_target = y_target - y_target_start;

idx_t0 = find(y_target >= targetStartDistance, 1, 'first');
if isempty(idx_t0)
...
% === t_end-Ermittlung ===
idx_end = find(y_target > 0, 1, 'first');
if isempty(idx_end)
...
y_target_t0 = y_target(idx_t0);
y_target_tend = y_target(idx_end);
...
% xline(0, '--k', 't_0 @ -11,11 m', 'LabelVerticalAlignment', 'top', 'LabelHorizontalAlignment', 'left');
...
% xTargetDist = xTargetDist;
withinTol6 = all(abs(x_target(t_rel >= 0 & t_rel <= (t_end - t_0)) - xTargetDes) <= tolLatTarget);

p1_2_2 = fill([0, t_end - t_0, t_end - t_0, 0], ...
     [xTargetDes + tolLatTarget, xTargetDes + tolLatTarget, ...
      xTargetDes - tolLatTarget, xTargetDes - tolLatTarget], ...
     withinTol6*[0.7 1 0.7] + ~withinTol6*[1 0.8 0.8], ...
     'EdgeColor', 'none', 'FaceAlpha', 0.5, 'DisplayName', 'Toleranzbereich');

p1_2_1 = plot(t_rel, x_target, 'r', 'LineWidth', 1.5);

% xline(0, '--k', 't_0 @ -11,11 m', 'LabelVerticalAlignment', 'bottom', 'LabelHorizontalAlignment', 'left');
% xline(t_end - t_0, '--k', 't_{end}', 'LabelVerticalAlignment', 'bottom', 'LabelHorizontalAlignment', 'left');
...
ylim([xTargetDes - tolLatTarget - 0.2, xTargetDes + tolLatTarget + 0.2]);

% Y-Ticks festlegen
yt = ylim;
yt_new = yt(1):0.2:yt(2);
yticks(yt_new);

% Formatierte Y-Tick-Beschriftungen setzen
%yticklabels(arrayfun(@(y) sprintf('%.2f', y), yt_new, 'UniformOutput', false));
...
legend({'VUT', 'Target'}, 'Location', 'east');
...
if printPlots == 1
    ...
    %----------------------------------------------------------------------
    filename     = sprintf("Static1_Plot2");
...
withinTol7 = all(abs(x_target(y_target >= y_target_t0 & y_target <= y_target_tend) - xTargetDes) <= tolLatTarget);

p2_1_2 = fill([y_target_t0, y_target_tend, y_target_tend, y_target_t0], ...
     [xTargetDes + tolLatTarget, xTargetDes + tolLatTarget, ...
      xTargetDes - tolLatTarget, xTargetDes - tolLatTarget], ...
     withinTol7*[0.7 1 0.7] + ~withinTol7*[1 0.8 0.8], ...
     'EdgeColor', 'none', 'FaceAlpha', 0.4, 'DisplayName', 'Toleranzbereich');

p2_1_1 = plot(y_target, x_target, 'r', 'LineWidth', 1.5);

% xline(y_target_t0, '--k', 't_0 @ -11,11 m', 'LabelVerticalAlignment', 'top', 'LabelHorizontalAlignment', 'left');
% xline(y_target_tend, '--k', 't_{end}', 'LabelVerticalAlignment', 'top', 'LabelHorizontalAlignment', 'left');

xline(y_target_t0, '--k', 't_0', 'LabelVerticalAlignment', 'bottom', 'LabelHorizontalAlignment', 'left','DisplayName', 't_{0} = Test Start');
xline(y_target_tend, '--k', 't_{end}', 'LabelVerticalAlignment', 'bottom', 'LabelHorizontalAlignment', 'right','DisplayName', 't_{End} = Test End');

xlabel('Y-Position Target [m]');
ylabel('Lat. Pos. Target [m]');
%title('Y position of the target over X position of the target');
%legend('Target trajectory', 'Location', 'best');
%legend([p2_1_1, p2_1_2], {'Target X-Position', 'Tolerance'}, 'Location', 'north');

xlim([y_target_t0-2, y_target_tend+2]);
ylim([xTargetDes - tolLatTarget - 0.2, xTargetDes + tolLatTarget + 0.2]);
...
xfill_target = [y_target_t0, y_target_tend];
yfill_target = [speedTarget - targetTol, speedTarget + targetTol];
...
p2_2_1 = plot(y_target, speed_target, 'r', 'LineWidth', 1.5);
...
text(y_target_t0 + 0.2, speedTarget + targetTol + 0.2, sprintf('Target Speed at 11.11 m: %.2f km/h', speed_target(idx_t0)), ...
    'HorizontalAlignment', 'left', ...
...
% Logischer Indexbereich im Positionsvektor (x_target_neg) zwischen dc und dd
idx_region = y_target <= y_end;
...
if any(warnsignal_filtered(idx_region))
    ...
    p2_3_2 = fill([y_target_t0 y_end y_end y_target_t0], [yl(1) yl(1) yl(2) yl(2)], ...
...
% xline(y_target_t0, '--k', 't_0 @ -11.11 m', 'LabelVerticalAlignment', 'middle', 'LabelHorizontalAlignment', 'left','DisplayName', 't_{0} = Test Start');
% xline(y_target_tend, '--k', 't_{end}', 'LabelVerticalAlignment', 'middle', 'LabelHorizontalAlignment', 'left','DisplayName', 't_{End} = Test End');
% xline(lip, '--k', 'LIP @ -2,0 m', 'LabelVerticalAlignment', 'middle', 'LabelHorizontalAlignment', 'left');
...
plot(y_target, warnsignal, 'Color', [0.5 0.5 0.5], 'DisplayName', 'Raw Signal');
hold on; grid on;
p2_3_1 = plot(y_target, warnsignal_filtered, 'b', 'LineWidth', 1.5, 'DisplayName', 'Filtered Signal');

% Erstes Auftreten von 1 im gefilterten Warnsignal
firstIdx = find(warnsignal_filtered, 1, 'first');

if ~isempty(firstIdx)
    y_first = y_target(firstIdx);

    % Punkt markieren
    plot(y_first, 1, 'ko', 'MarkerFaceColor', 'k', 'DisplayName', 'First Signal Activation');

    % Beschriftung des Punktes (z. B. oberhalb)
    text(y_first - 0.1, 0.95, sprintf('%.2f m', y_first), ...
...
% % Farbfläche zeichnen (nur Hintergrund)
% yl = ylim;  % aktueller Y-Achsenbereich
% fill([x_start x_end x_end x_start], [yl(1) yl(1) yl(2) yl(2)], ...
%      color_shade, 'EdgeColor', 'none', 'FaceAlpha', 0.5, 'DisplayName', 'Signal');
...
if printPlots == 1
    ...
    % 1) Speicherpfad und Dateinamen definieren
    %----------------------------------------------------------------------
    filename     = sprintf("Static1_Plot1");
    outputFolder = "C:\Users\G00550\Desktop\Elias Daten\MATLAB\R151 Auswertung\Entwicklung\PDF_Ausgabe_Test";
    if ~exist(outputFolder, "dir"); mkdir(outputFolder); end
    filePDF = fullfile(outputFolder, filename + ".pdf");
...
%         axLogo.YDir = 'normal';  % notwendig, da 'image' Y-Achse invertieren kann
    ...
    %----------------------------------------------------------------------
    if ishandle(993)
        figSrc = figure(993);  % vorhandene Plot-Figure
        % komplette Achsen und Inhalte in neue Figure kopieren
        axChildren = findall(figSrc, 'Type', 'axes');
...
%                 set(newAxes, 'Color', 'w');  % Hier explizit auf Weiß setzen
        ...
        warning("Figure 993 existiert nicht – Sammelplot konnte nicht übernommen werden.");
    end
...
fill([y_target_t0, y_target_tend, y_target_tend, y_target_t0], ...
     [1.15 + tolLatTarget, 1.15 + tolLatTarget, 1.15 - tolLatTarget, 1.15 - tolLatTarget], ...
     color_shade, 'EdgeColor', 'none', 'FaceAlpha', 0.5, ...
     'DisplayName', '±0.2 m Toleranz');

% Linien für ±0.2 m
%yline(yTargetDes + tolLatTarget, 'LineStyle', '-', 'Color', [0.2 0.2 0.2], 'LineWidth', 1.0, 'DisplayName', '+0.2 m');
%yline(yTargetDes - tolLatTarget, 'LineStyle', '-', 'Color', [0.2 0.2 0.2], 'LineWidth', 1.0, 'DisplayName', '-0.2 m');
%yline(0, '-k', 'LineWidth', 1.0, 'DisplayName', '0 m');

p51_1 = plot(y_target, x_vut, 'b', 'LineWidth', 1.5);
p51_2 = plot(y_target, x_target, 'r', 'LineWidth', 1.5);

xline(y_target_t0, '--k', 't_0', 'LabelVerticalAlignment', 'bottom', 'LabelHorizontalAlignment', 'left');
xline(y_target_tend, '--k', 't_{end}', 'LabelVerticalAlignment', 'bottom', 'LabelHorizontalAlignment', 'left');
yline(0);
%yline(1.15,'-', '1.15')
xlabel('Y-Position Target [m]');
ylabel('X-Position [m]');
...
extraTicks = [0, 1.15, 1.15 + 0.2, 1.15 - 0.2];
...
legend([p51_1, p51_2], {' VUT', 'Target'}, Location="east");
...
if printPlots == 1
    ...
    % 1) Speicherpfad und Dateinamen definieren
    %----------------------------------------------------------------------
    filename     = sprintf("Static1_Plot3");
    outputFolder = "C:\Users\G00550\Desktop\Elias Daten\MATLAB\R151 Auswertung\Entwicklung\PDF_Ausgabe_Test";
    if ~exist(outputFolder, "dir"); mkdir(outputFolder); end
    filePDF = fullfile(outputFolder, filename + ".pdf");
...
%         axLogo.YDir = 'normal';  % notwendig, da 'image' Y-Achse invertieren kann
    ...
    %----------------------------------------------------------------------
    if ishandle(5111)
        figSrc = figure(5111);  % vorhandene Plot-Figure
        % komplette Achsen und Inhalte in neue Figure kopieren
        axChildren = findall(figSrc, 'Type', 'axes');
        legChildren = findall(figSrc, 'Type', 'legend');
        ...
        warning("Figure 5111 existiert nicht – Sammelplot konnte nicht übernommen werden.");
    end
